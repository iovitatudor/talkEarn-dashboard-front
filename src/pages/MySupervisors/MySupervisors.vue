<template>
  <div class="tables-basic">
    <b-breadcrumb>
      ...
      <b-breadcrumb-item active>My Supervisors</b-breadcrumb-item>
    </b-breadcrumb>
    <b-row>
      <b-col>
        <b-row>
          <b-col>
            <h2 class="page-title">My Supervisors</h2>
          </b-col>
        </b-row>
        <Widget customHeader>
          <div class="table-responsive">
            <h5 v-if="!supervisorExperts.length" class="text-center">
              You don't have supervisors yet!
            </h5>
            <table class="table table-striped" v-else>
              ...
              <tbody>
              <tr v-for="(expert, key) in supervisorExperts" :key="key" v-if="expert.type !== 'Administrator'">
                <td>
                  <b>{{ key + 1 }}</b>
                </td>
                <td>
...
export default {
  ...
  methods: {
    ...
    async init() {
      try {
        const result = await ExpertApi.getExpertSupervisors(this.defaultLanguage.abbr, this.authExpert.id);
        this.supervisorExperts = result.data.data;
      } catch (err) {
        SetApiError(err);
      }
    },
